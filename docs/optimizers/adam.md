...
m̂ ← m / (1 - β₁ᵗ)                    ← correction de biais
v̂ ← v / (1 - β₂ᵗ)
...
**La correction de biais** (`m̂`, `v̂`) corrige le fait qu'au début, `m` et `v` sont initialisés à zéro et mettent du temps à « chauffer ». Sans correction, les premières mises à jour seraient trop petites.
...
⚠️ **Si Adam diverge avec lr=0.001**, essaye 0.0001 plutôt que de toucher aux betas.
...
- **La division par `√v̂ + ε`** est élément-wise : chaque coefficient du paramètre a son lr effectif
